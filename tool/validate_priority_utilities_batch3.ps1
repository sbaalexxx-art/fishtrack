...
$changedDartFiles = @(
  ...
  "lib/features/figma_complete/presentation/figma_runtime_data.dart",
...
if ($LASTEXITCODE -ne 0) {
  Write-Warning "flutter doctor reported environment issues. Android gates below remain authoritative."
}
...
Write-Host "[3/8] Format Batch 3 files" -ForegroundColor Cyan
...
Invoke-CheckedNative -Executable "flutter" -ArgumentList @("analyze") -FailureMessage "flutter analyze failed. Batch 3 was not built or installed."

Write-Host "[5/8] Priority utility integration tests" -ForegroundColor Cyan
...
) -FailureMessage "Batch 3 priority utility tests failed. Batch 3 was not built or installed."

Write-Host "[6/8] Water, Map, Weather and shell regression tests" -ForegroundColor Cyan
Invoke-CheckedNative -Executable "flutter" -ArgumentList @(
  "test",
  "test/home_navigation_shell_test.dart",
  "test/shell_runtime_navigation_test.dart",
  "test/map_batch1_regression_test.dart",
  "test/home_water_presentation_test.dart",
  "test/water_mobile_contract_test.dart",
  "test/weather_page_section_test.dart",
  "test/runtime_integrity_test.dart",
  "test/fluviai_responsive_components_test.dart",
  "test/figma_complete_contract_test.dart"
) -FailureMessage "Batch 3 regression tests failed. Batch 3 was not built or installed."
...
if ($Install) {
  ...
  $qaDirectory = Join-Path $root "build\qa\batch3"
  New-Item -ItemType Directory -Path $qaDirectory -Force | Out-Null
  $remoteScreenshot = "/sdcard/fluviai_batch3_priority_utilities.png"
  $localScreenshot = Join-Path $qaDirectory "priority_utilities_after_standalone_launch.png"
  ...
  & $adb @adbArgs logcat -d -t 800 | Out-File -FilePath $logPath -Encoding utf8
  if ($LASTEXITCODE -ne 0) {
...
Write-Host "Batch 3 validation completed." -ForegroundColor Green
