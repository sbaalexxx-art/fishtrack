-- Hydro Dispatch runtime health v2 source-policy repair.
--
-- Reassert the approved commercial serving policy after production drift:
-- Day-Ahead market readiness is counted from ENTSOE_DA_RO only.
-- Candidate OPCOM_PZU data must not satisfy product-serving health until an
-- explicit written commercial-reuse approval is obtained and separately gated.
-- This migration deletes no data and does not change the P0.3 fail-soft rules.
