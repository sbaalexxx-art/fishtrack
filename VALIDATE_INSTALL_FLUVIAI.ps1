...
$outputDir = Join-Path $projectRoot "outputs\figma_complete_rc4_1_$stamp"
New-Item -ItemType Directory -Path $outputDir -Force | Out-Null

Write-Host "[preflight] RC4.1 source integrity" -ForegroundColor Cyan
$pythonCommand = Get-Command python -ErrorAction SilentlyContinue
if ($null -ne $pythonCommand) {
    & $pythonCommand.Source (Join-Path $projectRoot "tool\verify_figma_complete_rc4_1.py")
    if ($LASTEXITCODE -ne 0) { throw "RC4.1 static preflight failed." }
}
else {
    Write-Host "Python not found; Flutter analyze remains the authoritative source check." -ForegroundColor Yellow
}
...
$remoteScreenshot = "/sdcard/fluviai_figma_complete_rc4_1_1_$stamp.png"
...
Copy-Item -LiteralPath $apk -Destination (Join-Path $outputDir "FluviAI-Figma-Complete-RC4.1-debug.apk") -Force

Write-Host ""
Write-Host "FLUVIAI FIGMA COMPLETE RC4.1 TECHNICAL PIPELINE COMPLETED" -ForegroundColor Green
